...
- **Design only.** Never write code from this skill. Never invoke `implement-prd` — that runs in a separate session.

## Phase 1 — Resolve target

- **`working_dir`** — absolute path to the project root. Default `cwd`. Ask once only if `cwd` is not a project root (no `go.mod`, no `package.json`) or the feature targets a different project.
...
Render the PRD inline and ask the user, in their language, what is wrong, missing, or worth adding before this becomes the working document. Apply edits. Close with one line pointing them at `implement-prd` against the file in a fresh session.
