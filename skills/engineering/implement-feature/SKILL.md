...
Record the spec locator, diff base revision, approved clarifications, and explicit non-goals.
...
Give a code-writer subagent the spec locator, base revision, scope, and any prior arbitration record locator; use the configured `code-writer` when available.
Require it to read the spec directly and return a local diff with check results; do not substitute a retold spec for the source.
On the stable diff, run the project's required checks, including the full suite when mandatory; record commands, results, and environment.
...
Run `/review-changes` with the spec locator and base revision, then run `/analyze-review-issues` on every review report.
