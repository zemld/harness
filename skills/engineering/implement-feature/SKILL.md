...
description: Drives a complete code-writing flow from a structured spec by running the chunk through the pipeline defined for its stack in `docs/engineering/<stack>/index.md`. The spec is the design; this skill is pure execution. Use when the caller has a finished design spec (intent, working_dir, files, interfaces) and wants verified, formatted code. Trigger on "implement this feature", "run the implementation flow", "execute the spec", "реализуй фичу по спеке", "запусти имплементацию", or when an upstream skill hands off a completed design spec. Do NOT use for design or decomposition — those are upstream concerns the spec must already resolve.
---
